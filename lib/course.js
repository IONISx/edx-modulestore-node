...
var Course = Base.extend({
    ...
    */
    ...
    initialize: function (modulestore, options) {
        ...
        this._model = options.model || new models.Course();
        this._about = {};
    ...
    */
    ...
    _fetchAboutFields: function (modulestore) {
        var self = this;
    ...
    attributes: {
        ...
        aboutPage: {
            get: function () {
                ...
                return [
                    this._modulestore.options.lmsUrl,
                    'courses',
                    this.id,
                    'about'
                ].join('/');
...
Course.list = function (modulestore, filter, options) {
    if (!modulestore) {
    ...
    models.Course.find(filter || {}, function (err, models) {
        if (err) {
            ...
            var courses = _.map(models, function (model) {
                return new Course(modulestore, _.extend({
                    model: model
                }, options));
            });
...
Course.get = function (modulestore, id, options) {
    if (!modulestore) {
        throw new Error('course: modulestore is required');
    }
    ...
    if (!courseId) {
        ...
        models.Course.findOne({
            ...
            if (err) {
                ...
                var course = new Course(modulestore, _.extend({
                    model: model
                }, options));
